...
export function cloneJSDocSignature(node: TS.JSDocSignature, options: CloneNodeInternalOptions<TS.JSDocSignature>): TS.JSDocSignature {
	...
	baseNode.typeParameters = options.hook(
		"typeParameters",
		cloneNodes(node.typeParameters, nextOptions(node.typeParameters, options)),
		node.typeParameters,
		payload(options)
	);
	baseNode.parameters = options.hook(
		"parameters",
		cloneNodes(node.parameters, nextOptions(node.parameters, options)),
		node.parameters,
		payload(options)
	);
